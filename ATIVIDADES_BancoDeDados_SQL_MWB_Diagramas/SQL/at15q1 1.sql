...
SELECT COUNT(funcao)AS quantidade from funcionarios where funcao like 'analista%';
SELECT funcionarios.nome as funcionariosmais1990 FROM funcionarios where extract( year from funcionarios.datanascimento)>1990;
SELECT round(avg(salario),2) as mediasalario FROM funcionarios ;
SELECT funcionarios.nome, date_format(datanascimento,'%d/%m/%y') as datanascimento,funcionarios.email FROM funcionarios where funcionarios.salario >5000 and funcionarios.salario < 7000 order by funcionarios.nome;
SELECT Departamentos.nome, sum(funcionarios.salario) as somasalarios FROM funcionarios,departamentos where funcionarios.codigodepartamento = departamentos.Codigo  group by departamentos.nome ; 
...
SELECT COUNT(funcao)AS quantidade from funcionarios;
select*from funcionarios where tempo_servico between 1 and 3;
select codigo, nome, salario from funcionarios order by nome;
select avg(salario)from funcionarios where tempo_servico > 3;
select departamentos.nome, funcionarios.nome, funcionarios.salario from departamentos, funcionarios where funcionarios.CodigoDepartamento = departamentos.Codigo order by funcionarios.Salario; 
SELECT Departamentos.nome FROM departamentos,funcionarios where funcionarios.codigodepartamento = departamentos.Codigo group by departamentos.nome having count(departamentos.nome) >= 2 ; 
select*from funcionarios where day (inicio_ferias) in (07, 09, 11) order by month(inicio_ferias) desc;
select count(nome)as funcionarios_a from funcionarios where nome like 'a%';
