...
delimiter &
create procedure func_salario (in salario decimal(10,2))
begin
...
&
call func_salario(5000);
select * from Funcionarios;

delimiter & 
create procedure  mudanca_departamento (in nome varchar(255),in quantidade int, in codigo int) 
begin  
...
&
call mudanca_departamento('Vendas',3,115); 
select * from departamentos;

delimiter & 
create procedure  Gerencia_Departamento (in coddep INT, in nome varchar(225), in loc varchar(225), in qf INT) 
begin  
INSERT INTO departamentos values (coddep,nome,loc,qf);
end 
& 
delimiter &
call Gerencia_Departamento(116, 'Gerencia', 'Ribeirão Preto', 1);
&

delimiter &
create procedure exc_Func (in codfunc INT)
begin
...
delimiter &
create procedure nova_gerencia (in cod INT, in nome varchar(225),in sobrenome varchar(225), in coddep INT)
begin
  declare qdepf int;
  select count (funcionarios.codigo)into qdepf from departamentos where funcionarios.codigodepartamento = coddep;
  
  if (select departamentos.quantidadefuncionario from departamentos where departamentos.codigo = coddep) > qdepf then
  insert into funcionarios(codigo, nome, sobrenome, codigodepartamento) values (cod, nome, sobrenome, coddep);
  else 
  select 'vagas ocupada';
  end if;
  
end;
&

call nova_gerencia (217, 'John', 'Aguir', 118);

delimiter &
create procedure mediaDep(id int, idfuncionario int)
begin
	while (select avg(Salario) from funcionarios where id = funcionarios.CodigoDepartamento) > (select Salario from funcionarios where Codigo = idfuncionario) do
		update funcionarios set Salario = (Salario + (Salario * 0.2)) where Codigo = idfuncionario;
	end while;
end; 
&

call mediaDep(102, 202);

delimiter &
create procedure novoDep(idfuncionario int, id int)
begin
	if id <> (select CodigoDepartamento from funcionarios where idfuncionario = funcionarios.Codigo) then
	update funcionarios set CodigoDepartamento = id where idfuncionario = funcionarios.Codigo;
    else
    select 'O funcionário já está neste departamento' as mensagem;
    end if;
end;
&

call novoDep(202, 103);

delimiter &
create procedure older()
begin
	select Nome, DataNascimento from funcionarios where year(now()) - (year(Datanascimento)) < 30 order by DataNascimento;
end; 
&
...
delimiter &
create procedure depeb()
begin
	select funcionarios.Nome, Departamentos.localizacao from funcionarios, departamentos
    where (localizacao like 'B%' and localizacao like '%A') and funcionarios.codigodepartamento = departamentos.codigo;
end; 
&
...
delimiter &
create function qtade_fun() returns int 
begin 
   declare qtade int;
   select count(*) into qtade from funcionarios;
   return qtade;
end;
&
...
DELIMITER &
 CREATE FUNCTION func_sm(id int) RETURNS decimal
 BEGIN 
...
DELIMITER &
 CREATE FUNCTION func_idmin(nome varchar(55)) RETURNS int
 BEGIN 
...
&
delimiter &
select func_idmin() as Funcionario_mais_Novo;
&

DELIMITER &
 CREATE FUNCTION soma_Deps (id int) RETURNS decimal
 BEGIN 
...
&
delimiter &
select soma_Deps(102);
&

DELIMITER &
 CREATE FUNCTION niveldep (id int, valor decimal(10, 2)) RETURNS varchar(55)
 BEGIN 
...
&
delimiter &
select niveldep (101, 14500);
&

DELIMITER &
 CREATE FUNCTION novosal (id int, valor decimal(10, 2)) RETURNS varchar(55)
 BEGIN 
     declare sal decimal(10, 2);
