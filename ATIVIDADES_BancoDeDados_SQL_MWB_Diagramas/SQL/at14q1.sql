...
alter table aluno add telefone varchar(15) after nome;
...
(2, 'engenharia civil', 'diurno'),
(3, 'administração', 'noturno'),
(4, 'psicologia', 'noturno'),
(5, 'engenhria elétrica', 'diurno'),
(6, 'marketing', 'noturno');
select * from Curso;
 
insert into Professor values (201, 'ana souza', 1112223333, 'rua X, 789', 'ana@gmail.com', 1),
(202, 'pedro rocha', 4445556666, 'avenida Y, 456', 'pedro@gmail.com', 2),
(203, 'laura lima', 9998887777, 'praça Z, 123', 'laura@gmail.com', 3),
(204, 'rodrigo lima', 5556667777, 'rua W, 432', 'rodrigo@gmail.com', 4),
(205, 'mariana rocha', 1115559999, 'avenida V, 876', 'mariana@gmail.com', 5),
(206, 'lucas pereira', 7773332222, 'praça U, 123', 'lucas@gmail.com', 6);
...
select nome, email, telefone from professor order by nome;
select nomeAluno, email, telefone from aluno order by email;
select * from professor;
select aluno.matricula, aluno.nome, aluno.professor_matricula, professor.nome ,professor.codCurso, curso.nomeCurso from aluno, professor, curso where aluno.professor_matricula = professor.matricula2 and professor.codCurso = curso.codCurso;
select professor.matricula2, professor.nome, professor.codCurso, curso.nomeCurso from professor, curso where professor.codCurso = curso.codCurso order by nome;
select nome from professor where email like 'l%';